...
def apply_fastICA(dd, n_comp=None, algo="parallel", seed=42):
    # Deprecation warning for apply_ICA, use apply_fastICA instead
    
    warnings.warn("apply_fastICA is deprecated, use fmcg.ica.apply_ICA instead", DeprecationWarning)
...
def plotHR(signal, compF=None, fs=1000, minBpm=-1, maxBpm=-1, plot=True):
    warnings.warn("plotHR is deprecated, use fmcg.analysis.hr.compute_hr instead", DeprecationWarning)

    def _find_peaks(signal):
        # Find all peaks
        signal, _ = nk.ecg_invert(signal, fs) if signal.shape[0] > 2*fs else (signal, None)
...
def identify_hr_range_segments(
    ...
    warnings.warn("identify_hr_range_segments is deprecated, use the workflow in fmcg.analysis.hr instead", DeprecationWarning)
...
def __avg_hr_range(
    ...
    warnings.warn("__avg_hr_range is deprecated, use the workflow in fmcg.analysis.heartbeat_averaging instead", DeprecationWarning)
