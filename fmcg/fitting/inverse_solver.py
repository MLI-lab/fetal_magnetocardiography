...
class InverseSolver:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.method = method
        if method in ["basinhopping", "bfgs"]:
            self.parameter_format = "vector"
        elif method in ["adam", "twostep", "lamb"]:
    ...
    def _setup_schedulers(self, optimizer, lr_config):
        """Setup warmup and plateau schedulers."""
        warmup = lr_config.get("warmup", 50)
        patience_values = lr_config.get("patience_groups", [])
        patience = patience_values[0] if patience_values else 20

        n_groups = len(optimizer.param_groups)
        warmup_fn = lambda epoch: (1 - np.cos(np.pi * (epoch + 1) / max(warmup, 1))) / 2
        warmup_scheduler = optim.lr_scheduler.LambdaLR(
            optimizer, lr_lambda=[warmup_fn] * n_groups, last_epoch=-1
        )
        plateau_scheduler = optim.lr_scheduler.ReduceLROnPlateau(
            optimizer, mode='min', factor=0.9, patience=patience, threshold=1e-5,
        )
        logger.debug(f"Warmup for {warmup} steps, Plateau patience={patience}")
        return {
    ...
    def loss(self, parameters, field_true, warmup_done=False, return_numpy=False, skip_backward=False, **kwargs):
        ...
        # Always store component breakdown for external access (e.g. bfgs callback logging)
        self._last_loss_components = {k: v.item() if isinstance(v, torch.Tensor) else v for k, v in loss_dict.items()}
        self._last_loss_components["loss"] = loss.item()

        if not skip_backward:
            loss.backward()
            self.callback.log(**self._last_loss_components, **kwargs)
        if return_numpy:
        ...
        if self.method in ["adam", "twostep", "lamb"]:
            ...
            with logging_redirect_tqdm():
                for i in (
                    ...
                    current_loss = None
                    def closure():
                        nonlocal best_loss, best_parameters, best_iteration, current_loss
                        optimizer.zero_grad()
                        ...
                        if loss < best_loss and i >= schedulers["warmup_steps"]:
                            best_loss = current_loss
                            best_parameters = [p.detach().clone() for p in parameters] if isinstance(parameters, list) else parameters.detach().clone()
                            best_iteration = i
                    ...
                    if i < schedulers["warmup_steps"]:
                        ...
                        schedulers["plateau"].step(current_loss)
                        current_lrs = [group["lr"] for group in optimizer.param_groups]
                        lr_reduced = any(cur < prev for cur, prev in zip(current_lrs, prev_lrs))
                    ...
                    if early_stop and i > schedulers["warmup_steps"]:
                        ...
                        if current_loss < early_stopping_best_loss - early_stopping_threshold:
                            early_stopping_best_loss = current_loss
                            early_stopping_counter = 0
            ...
            torchmin_method = kwargs.pop("torchmin_method", "l-bfgs")
            max_iter = lr.get("niter", 200)
            warmup_steps = lr.get("warmup", 0)

            x0 = parameters.detach().clone()
            self.best_loss = float("inf")
            self.best_parameters = None
            self.best_i = 0
            step = [0]

            def _make_fun(warmup_done):
                def fun(x):
                    return self.loss(x, field_true, skip_backward=True, warmup_done=warmup_done)
                return fun

            def _make_callback(warmup_done):
                def callback(x):
                    loss_val = self.loss(x, field_true, skip_backward=True, warmup_done=warmup_done).item()
                    if loss_val < self.best_loss:
                        self.best_loss = loss_val
                        self.best_parameters = x.detach().clone()
                        self.best_i = step[0]
                        pbar.set_postfix({"Best Loss": self.best_loss})
                    self.callback.log(**self._last_loss_components)
                    pbar.update(1)
                    step[0] += 1
                return callback

            with logging_redirect_tqdm():
                total = warmup_steps + max_iter if warmup_steps > 0 else max_iter
                pbar = trange(total, desc="Iterations", leave=True)

                # Phase 1: warmup — optimize without penalty terms (fresh Hessian)
                if warmup_steps > 0:
                    result = torchmin_minimize(
                        _make_fun(warmup_done=False), x0,
                        method=torchmin_method,
                        max_iter=warmup_steps,
                        callback=_make_callback(warmup_done=False),
                    )
                    x0 = result.x.detach().clone()

                # Phase 2: full loss with fresh Hessian approximation
                result = torchmin_minimize(
                    _make_fun(warmup_done=True), x0,
                    method=torchmin_method,
                    max_iter=max_iter,
                    callback=_make_callback(warmup_done=True),
                )
                pbar.close()

            if self.best_parameters is None:
                self.best_parameters = result.x
                self.best_loss = result.fun if isinstance(result.fun, float) else result.fun.item()
                self.best_i = result.nit
    ...
    def solve_with_config(self, field_true, initial_parameters, config, **kwargs):
        ...
        if "gaussian_field_prior" in config:
            loss_args["gaussian_field_prior"] = config["gaussian_field_prior"]
        if "gaussian_moment_prior" in config:
            loss_args["gaussian_moment_prior"] = config["gaussian_moment_prior"]
