...
import numpy as np
import pywt
from scipy.signal import butter, filtfilt, iirnotch, sosfiltfilt, decimate
...
def downsample_peaks(peaks: np.ndarray, from_fs: float, to_fs: float, max_len: int) -> np.ndarray:
    """Rescale peak indices between sample rates and clip to valid bounds."""
    scaled = np.round(np.asarray(peaks).astype(float) * (float(to_fs) / float(from_fs))).astype(int)
    return scaled[(scaled > 0) & (scaled < int(max_len))]


def downsample(datasets, fs, fs_target=250):
    """
    Downsample the input dataset(s) to a target sampling frequency.

    Parameters:
        datasets (numpy.ndarray or list of numpy.ndarray): The input dataset(s) to be downsampled. Can be a single 2D array (n_samples, n_channels) or a list of such arrays.
        fs (int): The original sampling frequency of the dataset(s).
        fs_target (int, optional): The target sampling frequency to downsample to. Default is 250 Hz.

    Returns:
        numpy.ndarray or list of numpy.ndarray: The downsampled dataset(s) with the same format as the input (single array or list of arrays).
        int: The new sampling frequency after downsampling (should be equal to fs_target).
    """
    q = fs // fs_target
    if fs % fs_target != 0:
        raise ValueError(
            f"Target sampling frequency {fs_target} must be an integer divisor of the original sampling frequency {fs}."
        )
    if q == 1:
        logger.warning(
            f"Original sampling frequency {fs} is equal to target sampling frequency {fs_target}. No downsampling applied."
        )
        return datasets, fs
    if q <= 1:
        raise ValueError(
            f"Original sampling frequency {fs} must be greater than target sampling frequency {fs_target}."
        )

    # input can be either a single array or a list of arrays
    is_single_array = isinstance(datasets, np.ndarray)
    if is_single_array:
    ...
    # Process the data
    downsampled_data = [decimate(data, q, axis=0) for data in datasets_list]

    # Return in the same format it was received
    if is_single_array:
        return downsampled_data[0], fs // q
    else:
        return downsampled_data, fs // q
...
def filternk(sig, fs, method="biosppy", **kwargs):
    ...
    for i in range(sig.shape[1]):
        ...
        sig_filt[nan_mask, i] = nk.ecg_clean(
            sig[nan_mask, i], sampling_rate=fs, method=method, **kwargs
        )
    return sig_filt
...
def wavelet_denoise(
    signal, wavelet="db6", level=None, threshold=None, plot=False, verbose=True
):
    """
...
def pca_filter(data, remove_first=3, verbose=False):
    ...
    for i in range(pca_filtered.shape[-1]):
        ...
        c[:, : remove_first[i]] = 0
        pca_filtered[:, np.isnan(pca_filtered[:, :, i]).all(axis=0) == False, i] = (
            np.dot(c, pca.components_)
        )
...
def dict_remove_median(sensor_dict, axis=0, window=None):
    ...
    if window is None:
        common_mode = np.nanmedian(
            np.array(list(sensor_dict.values())), axis=axis, keepdims=False
        )
        return {
            name: sensor_dict[name] - common_mode for name in sensor_dict.keys()
        }, common_mode
    ...
    return {
        name: sensor_dict[name] - common_mode[: sensor_dict[name].shape[0]]
            ...
            for j in range(3):
                for k in range(window_data.shape[1]):
                    ...
                    reg = LinearRegression().fit(
                        agg[:, 0, j].reshape(-1, 1), window_data[:, k, j]
                    )
                    beta = reg.coef_[0]
                    ...
                    field_cmr[i * window : (i + 1) * window :, k, j] -= (
                        beta * agg[:, 0, j]
                    )
