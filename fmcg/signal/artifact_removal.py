...
def estimate_noise_mask(
    ...
    low_cut: float = 3.0, 
    hi_cut: float = 45.0, 
    threshold_scale_factor: float = 5.0, 
    data_interval: float = 1.0, 
    consolidation_spacing: float = 10.0,
    plot: bool = False
) -> np.ndarray:
    """
    Estimates a binary mask of low-frequency noise bursts based on the Saki algorithm.
    
    Args:
        data: 1D numpy array containing the raw signal data.
        fs: Sampling frequency of the data in Hz.
        low_cut: Low cutoff frequency for the bandpass filter (Hz).
        hi_cut: High cutoff frequency for the bandpass filter (Hz).
        threshold_scale_factor: Multiplier for the filtered data statistic.
        data_interval: Length of the analysis windows in seconds.
        consolidation_spacing: Maximum spacing in seconds to merge adjacent intervals.
        plot: If True, displays a matplotlib figure of the results. Defaults to False.
        
    Returns:
        A 1D boolean numpy array (mask) of the same length as `data`.
        True indicates noise, False indicates clean data.

    References:
    - Saki User's Manual 2020, Biomagnetism Lab, University of Wisconsin-Madison
    """
    
    # 1. Bandpass filter the signal
    nyquist = 0.5 * fs
    b, a = butter(N=4, Wn=[low_cut / nyquist, hi_cut / nyquist], btype='bandpass')
    filtered_data = filtfilt(b, a, data)
    
    # 2. Derive statistics of the filtered data
    filtered_std = np.std(filtered_data)
    threshold = filtered_std * threshold_scale_factor
    
    # 3. Process the unfiltered data in intervals
    samples_per_interval = int(data_interval * fs)
    num_intervals = len(data) // samples_per_interval
    
    # Initialize the full-length output mask to False
    mask = np.zeros(len(data), dtype=bool)
    
    if num_intervals == 0:
        return mask
        
    # Truncate data to fit exactly into shaped intervals, then reshape into a 2D array
    truncated_data = data[:num_intervals * samples_per_interval]
    reshaped_data = truncated_data.reshape((num_intervals, samples_per_interval))
    
    # Vectorized mean calculation across the intervals
    interval_means = np.mean(reshaped_data, axis=1)
    
    # Create an interval-level boolean mask
    noisy_intervals = np.abs(interval_means) > threshold
    
    # 4. Consolidate interval pairs using Binary Closing
    max_gap_intervals = int(consolidation_spacing / data_interval)
    
    if max_gap_intervals > 0:
        structure_size = max_gap_intervals + 2
        structuring_element = np.ones(structure_size, dtype=bool)
        noisy_intervals = binary_closing(noisy_intervals, structure=structuring_element)
        
    # 5. Expand the interval-level mask back to the sample-level mask
    mask[:num_intervals * samples_per_interval] = np.repeat(noisy_intervals, samples_per_interval)

    if plot:
        _plot_noise_estimation(data, filtered_data, np.repeat(interval_means, samples_per_interval), threshold, mask, fs)
    
    return mask

def _plot_noise_estimation(data, filtered_data, expanded_means, threshold, mask, fs, ax=None):
    ...
    ax.plot(time, data, label='Raw Data', alpha=0.8, linewidth=.5)
    ax.plot(time, filtered_data, label='Filtered Data', alpha=0.8, linewidth=.5)
    ax.plot(time[:len(expanded_means)], expanded_means, label='Interval Means', color="tab:red", linewidth=1)
    
    # Plot both positive and negative thresholds
    ax.axhline(threshold, color='k', linestyle='--', label='+/- Threshold')
    ax.axhline(-threshold, color='k', linestyle='--')
    
    # Highlight the noise regions
    ax.fill_between(time, np.min(data), np.max(data), where=mask, color='red', alpha=0.3, label='Estimated Noise')
    
    ax.legend(loc='upper right')
    ax.set_xlabel('Time [s]')
    ax.set_title('Noise Estimation')
...
def remove_outlier_dict(
    sensor_dict, physical_range_threshold=0.1, threshold=0.5, remove_var_outlier=True, verbose=False
):
    """
    ...
    for k, v in sensor_dict.items():
        ...
        physical_mask[k] = chan_ranges > physical_range_threshold
    ...
    for k, v in sensor_dict.items():
        ...
        all_nan = np.isnan(v).all(axis=0)
        outlier_frac[all_nan] = 1.0
        outlier_frac[~physical_mask[k]] = 1.0
        results[k] = outlier_frac.round(2)
        #sensor_dict[k][:, (outlier_frac > threshold)] = np.nan #TODO Check if this is actually needed in practice
    ...
    if remove_var_outlier:
        for k, frac in results.items():
            ...
            sensor_dict[k][:, (frac > threshold)] = np.nan
