...
from fmcg.signal.artifact_removal import detect_artifacts, estimate_noise_mask
...
class Pipeline:
    ...
    def _plot_sensor_signals_if_enabled(
        self, data, time, ylabel, filename_suffix, xlim=None, **kwargs
    ):
        ...
        default_kwargs = {
            ...
            "sharey": True,
            "sensor_names": self.sensor_names or [],
        }
    ...
    def _execute_common_post_processing(self):
        ...
        for comp in self.data_dict:
            self.log_dict.update(
                _compute_hrv_quality_metrics(
                    self.data_dict, comp, self.fs_, self.config
                )
            )
    ...
    def _compute_gof_metrics_for(self, field_windowed, m_hat, r_hat):
        ...
        field_pred_phys = fm.forward_linear(
            r_tensor, m_tensor, as_numpy=True, silent=True
        )
        ...
        if self.noise_whitened is not None:
            ...
            discrepancy_threshold = (
                0.5 * np.mean(noise_norm_sq) / (np.mean(field_norm_sq) + 1e-12)
            )
            data_fit = (
                0.5 * np.mean(residual_norm_sq) / (np.mean(field_norm_sq) + 1e-12)
            )
            metrics.update(
                {
    ...
    def _load_raw_data(self):
        """
        Load raw signal and noise dicts from disk or a synthetic data_loader.

        Returns
        -------
        tuple: (sig_dict, time, fs, noise_dict, configs)
            configs contains r_sensors, MeasurementConfig, SystemConfig, etc.
        """
        self._log_step_header("Data Loading")

        if self.data_loader is not None:
            logger.info("Loading synthetic data...")
            sig_dict, time, fs, noise_dict = (
            ...
            if hasattr(self.data_loader, "params") and isinstance(self.data_loader.params, dict):
                ...
                if r_sensors is not None:
                    configs["r_sensors"] = np.array(r_sensors, copy=True)
        else:
        ...
        signal_length = len(list(sig_dict.values())[0]) / fs
        noise_length = len(list(noise_dict.values())[0]) / fs
        ...
        return sig_dict, time, fs, noise_dict, configs

    def _run_preprocessing(self, sig_dict, time, fs, noise_dict, r_sensors):
        """
        Preprocess loaded sensor dicts using the shared preprocess() function.

        Sets self.field_maps_windowed, self.time_windowed, self.axis_mask,
        self.W, self.r_sensors, self.fs_, self.artifacts_mask,
        self.noise_whitened, self.sensor_names, and self.processing_segments.
        """
        self._log_step_header("Data Preprocessing")

        params = {
            **self.config["preprocessing"],
            "sampfrom": self.config["data"].get("sampfrom", 0),
            "sampto": self.config["data"].get("sampto"),
        }
        if self.mask is not None:
            params["channel_mask"] = self.mask

        preprocessed = preprocess(sig_dict, noise_dict, fs, params, time=time, r_sensors=r_sensors)

        # Assign attributes expected by downstream methods
        self.field_maps_windowed = preprocessed.field_data
        self.field_maps_dec = preprocessed.field_data  # alias used in _combine_segment_results
        self.time_windowed = preprocessed.time
        self.axis_mask = preprocessed.axis_mask
        self.W = preprocessed.whitening_matrix
        self.r_sensors = preprocessed.r_sensors
        self.fs_ = preprocessed.fs
        self.artifacts_mask = preprocessed.artifacts_mask
        self.sensor_names = preprocessed.sensor_names

        # Noise in whitened space — needed for GOF discrepancy metrics
        if preprocessed.whitening_matrix is not None and preprocessed.noise_maps is not None:
            noise_valid = preprocessed.noise_maps[:, preprocessed.axis_mask == 1].copy()
            noise_valid -= np.nanmean(noise_valid, axis=0)
            self.noise_whitened = noise_valid @ preprocessed.whitening_matrix.T
        else:
            self.noise_whitened = None

        # Signal / noise power diagnostics
        valid_field = preprocessed.field_data[~preprocessed.artifacts_mask]
        self.sig_power = round(utils.sig_power(valid_field, root=True, win=10 * self.fs_), 4)
        if preprocessed.noise_maps is not None:
            noise_valid_all = preprocessed.noise_maps[:, preprocessed.axis_mask == 1]
            self.noise_power = round(utils.sig_power(noise_valid_all, root=True, win=10 * self.fs_), 4)
        else:
            self.noise_power = None

        # Segmented processing setup
        if self.config["preprocessing"].get("process_segments"):
            min_seg = int(self.config["preprocessing"].get("min_segment_length", 30) * self.fs_)
            self.processing_segments = _find_continuous_segments(
                preprocessed.artifacts_mask, min_length=min_seg
            )
            total_clean = sum(e - s for s, e in self.processing_segments) / self.fs_
            logger.info(
                f"Found {len(self.processing_segments)} clean segments "
                f"({total_clean:.1f}s total)"
            )
            self.log_dict["segments"] = [
                round((e - s) / self.fs_, 2) for s, e in self.processing_segments
            ]
            self.log_note += f"Found {len(self.processing_segments)} clean segments. "
        else:
            # Do NOT set self.processing_segments — _solve() uses hasattr() to detect segmented mode
            self.log_dict["segments"] = [round(len(preprocessed.field_data) / self.fs_, 2)]

        # Decimate ground truth arrays for synthetic data
        if hasattr(self, "ground_truth") and self.ground_truth:
            ...
            if q > 1:
                ...
                for key in ["m_true", "r_true", "field_clean", "time"]:
                    if key in gt and isinstance(gt[key], np.ndarray):
                        ...
                        logger.info(f"  {key}: {orig} -> {gt[key].shape}")
                if "fs" in gt:
                    gt["fs"] = self.fs_
        ...
        if self.save_plots:
            plot_signal(
                ...
                ylabel="Artifact",
                color="red",
                savename=os.path.join(self.output_dir, "1_artifacts_mask.pdf"),
            )
            self._plot_sensor_signals_if_enabled(
                preprocessed.field_data,
                preprocessed.time,
                ylabel="Filtered/Whitened Field [pT]",
    ...
    def _process_single_segment(self, segment_data, segment_time, segment_idx=0):
        ...
        m_hat, r_hat = _solve_segment(
            mdl, field_true, initial_parameters, self.config, segment_idx
        )
    ...
    def _initialize_solver(self):
        ...
        self.mdl, self.initial_parameters, self.field_true = (
            self._create_solver_components(self.field_maps_windowed)
        )
    ...
    def _solve(self):
        ...
        if hasattr(self, "processing_segments"):
            ...
            max_workers = self.config["solver"].get(
                "max_workers", min(4, len(self.processing_segments))
            )
            max_workers = min(
        ...
        self.log_dict["best_loss"] = float(self.mdl.best_loss)
        self.log_dict["best_i"] = (
            self.mdl.best_i.tolist()
            if isinstance(self.mdl.best_i, np.ndarray)
            else self.mdl.best_i
        )
    ...
    def _combine_segment_results(self, segment_data_dict, i, all_beats_data):
        ...
        for key in ["fetal", "maternal"]:
            ...
            if segment_start_sample > 0:
                if i > 0:
                    artifact_length = (
                        self.processing_segments[i][0]
                        - self.processing_segments[i - 1][1]
                    )  # TODO
                else:
                    artifact_length = segment_start_sample
                all_beats_data[key]["hr"].extend([np.nan] * artifact_length)
                all_beats_data[key]["position"].extend(
                    [[np.nan, np.nan, np.nan]] * artifact_length
                )
                all_beats_data[key]["artifacts"].extend([True] * artifact_length)
                all_beats_data[key]["dipole_moments"].extend(
                    [[np.nan, np.nan, np.nan]] * artifact_length
                )
            ...
            all_beats_data[key]["peaks"].extend(
                segment_data_dict[key]["peaks"] + segment_start_sample
            )
            ...
            all_beats_data[key]["artifacts"].extend(
                [False]
                * (self.processing_segments[i][1] - self.processing_segments[i][0])
            )
            all_beats_data[key]["dipole_moments"].extend(
                segment_data_dict[key]["dipole_moments"]
            )
            all_beats_data[key]["components"].extend(
                segment_data_dict[key]["components"]
            )
        ...
        if hasattr(self, "segment_results"):
            ...
            for i, result in enumerate(self.segment_results):
                if result is not None and "m_hat" in result:
                    ...
                    self._combine_segment_results(segment_data_dict, i, all_beats_data)
            ...
            logger.info(
                f"Collected {len(all_beats_data['fetal']['peaks'])} fetal beats and {len(all_beats_data['maternal']['peaks'])} maternal beats from all segments"
            )
            ...
            for component_name in self.heartbeats_dict.keys():
                if (
                    self.log_dict
                    and f"selected_beats_{component_name}" in self.log_dict
                    and f"selected_hr_{component_name}" in self.log_dict
                ):
                    logger.info(
                        f"Selected {component_name} {self.log_dict[f'selected_beats_{component_name}']:.2f} heartbeats with mean HR {self.log_dict[f'selected_hr_{component_name}']:.2f} bpm"
                    )
            ...
            if self.save_data:
                ...
                if hasattr(self, "ground_truth") and self.ground_truth:
                    save_dict["ground_truth"] = self.ground_truth
                    logger.info(
                        "Including ground truth in pipeline_results.pkl (segmented)"
                    )

                with open(
                    os.path.join(self.output_dir, "pipeline_results.pkl"), "wb"
                ) as f:
                    pickle.dump(save_dict, f)
    ...
    def _log_record(self):
        ...
        log_entry = {
            ...
            **{
            ...
            "outlier_signal": (
                ...
                if (
                    self.config["data"].get("noise_group_names", None) is not None
                    and hasattr(self, "sensor_dict")
                    and hasattr(self, "axis_mask")
                )
                else []
            ),
        }
        ...
        if (
            hr_f is not None
            and hr_m is not None
            and not np.isnan(hr_f)
            and not np.isnan(hr_m)
        ):
            log_entry["success"] = int(
                hr_f > hr_m + 10 and beats_f >= 5 and beats_m >= 5
            )
        else:
            log_entry["success"] = 0
    ...
    def _save_reports(self):
        ...
        if self.data_dict:
            ...
            logger.info(
                "Skipping detailed reports - insufficient data for segmented processing"
            )
    ...
    def load_measurements_from_labels(
        ...
        if measurement_config_date is not None:
            ...
            df_dates_all = pd.to_datetime(
                df["date"].astype(str).str.replace(r"^D", "", regex=True)
            )
        ...
        for i, row in df.iterrows():
            ...
            if (patient_labels == 0).any():
                ...
                has_empty = df["labels"].apply(
                    lambda x: any(np.array(ast.literal_eval(x), dtype=int) == 0)
                )
                if has_empty.any():
    ...
    def run(
        ...
        if (
            ...
            or measurement_config is not None
            or r_sensors is not None
        ):
            import warnings
        ...
        if hasattr(config, "to_dict"):
            ...
            logger.debug(
                "Converted PipelineConfig dataclass to dict format via dataclasses.asdict"
            )
        else:
            self.config = config
    ...
    def _pipeline_steps(self):
        ...
        sig_dict, time, fs, noise_dict, configs = self._load_raw_data()
        self._run_preprocessing(sig_dict, time, fs, noise_dict, configs.get("r_sensors"))
    ...
    def get_forward_solution(self, source_type="fetal"):
        """Compute the forward solution (field maps) based on the solved positions and head model."""
        if not hasattr(self, "r_hat") or self.r_hat is None:
            logger.warning("No solved positions (r_hat) available to compute forward solution.")
            return None
        if not hasattr(self, "mdl") or self.mdl is None:
            logger.warning("No model (mdl) available to compute forward solution.")
            return None
        ...
        if source_type == "fetal":
            ...
            logger.warning(f"Unknown source_type '{source_type}' for forward solution. Expected 'fetal' or 'maternal'.")
            return None

        return self.mdl.forward_model.forward_linear(self.r_hat[:, idx, :], self.m_hat[:, idx, :], as_numpy=True)[:, self.axis_mask] @ self.W.T
